...
def _pick_name(sec_name: Optional[str], short_name: Optional[str]) -> str:
    ...
    if sec_name:
        return sec_name[:255]
    return (short_name or "MOEX ETF")[:255]


def _fetch_moex_securities_table() -> dict:
    base_url = "https://iss.moex.com/iss/engines/stock/markets/shares/boards/TQTF/securities.json"
...
def _load_moex_securities() -> Iterable[dict[str, Optional[str]]]:
    ...
    json_path = base_dir / "marketdata" / "MOEX" / "tqtf_securities_snapshot.json"
    if json_path.exists():
...
def seed_moex_etf_tqtf(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")

    asset_type = (
        AssetType.objects.filter(code="etf_ru").first()
        or AssetType.objects.filter(name="ETF РФ").first()
        or AssetType.objects.filter(name="ETF").first()
    )
...
def unseed_moex_etf_tqtf(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_moex_etf_tqtf, reverse_code=unseed_moex_etf_tqtf),
    ]
