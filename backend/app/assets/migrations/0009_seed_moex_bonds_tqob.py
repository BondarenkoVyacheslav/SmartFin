...
def _pick_name(sec_name: Optional[str], short_name: Optional[str]) -> str:
    ...
    if sec_name:
        return sec_name[:255]
    return (short_name or "MOEX Bond")[:255]


def _fetch_moex_securities_table() -> dict:
    base_url = "https://iss.moex.com/iss/engines/stock/markets/bonds/boards/TQOB/securities.json"
    limit = 100
    start = 0
    columns: list[str] = []
...
def _load_moex_securities() -> Iterable[dict[str, Optional[str]]]:
    ...
    json_path = base_dir / "marketdata" / "MOEX" / "tqob_securities_snapshot.json"
    if json_path.exists():
...
def seed_moex_bonds_tqob(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")

    asset_type = (
        AssetType.objects.filter(code="bond_ru").first()
        or AssetType.objects.filter(code="bond").first()
        or AssetType.objects.filter(name="Облигации").first()
        or AssetType.objects.filter(name="Гособлигации").first()
    )
    ...
    for item in _load_moex_securities():
        ...
        currency = _normalize_currency(item.get("faceunit") or item.get("currency"))
        market_url = f"moex-bond:{symbol}"
...
def unseed_moex_bonds_tqob(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")
...
class Migration(migrations.Migration):
    dependencies = [
        ("assets", "0008_seed_moex_etf_tqtf"),
    ]

    operations = [
        migrations.RunPython(seed_moex_bonds_tqob, reverse_code=unseed_moex_bonds_tqob),
    ]
