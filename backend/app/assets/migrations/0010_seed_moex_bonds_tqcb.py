...
def _load_moex_securities() -> Iterable[dict[str, Optional[str]]]:
    ...
    local_json_path = migration_dir / "tqcb.json"
    legacy_local_json_path = migration_dir / "tqcb_data.json"
...
def seed_moex_bonds_tqcb(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")

    asset_type = (
        AssetType.objects.filter(code="bond").first()
...
def unseed_moex_bonds_tqcb(apps, schema_editor):
    AssetType = apps.get_model("assets", "AssetType")
    Asset = apps.get_model("assets", "Asset")
...
class Migration(migrations.Migration):
    dependencies = [
        ("assets", "0009_seed_moex_bonds_tqob"),
    ]

    operations = [
        migrations.RunPython(seed_moex_bonds_tqcb, reverse_code=unseed_moex_bonds_tqcb),
    ]
